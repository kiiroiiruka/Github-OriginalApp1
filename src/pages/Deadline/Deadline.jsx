...
const Deadline = () => {
  ...
  const calculateRemainingTime = (deadline) => {
    ...
    if (diffMs < 0)
      return { text: "締切済み", className: styles.red, blink: false };
    ...
    if (diffDays > 3) {
      ...
      return {
        text: `残り${diffMins}分${diffSecs % 60}秒`,
      ...
      <div className={styles.scrollContainer}>
        ...
        <VerticalList
          items={filteredMemos.map((memo) => {
            ...
            const { text, className, blink } = calculateRemainingTime(
              memo.deadline,
            );
            ...
            return {
              ...
              label: (
                <div onClick={() => handleNavigate(memo.id)}>
                  <div>{memo.title}</div>
                  <div
                    className={`${className} ${blink ? styles.blinkRed : ""}`}
                  >
